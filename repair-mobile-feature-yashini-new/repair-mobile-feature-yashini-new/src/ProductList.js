  ...
  const handleExcelUpload = (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const upsertPromises = json.map(async (row) => {
          const productData = {
            itemName: row['Item Name'],
            category: row['Category'],
            buyingPrice: parseFloat(String(row['Buying Price'] || 0).replace(/Rs\.?\s*/, '')),
            sellingPrice: parseFloat(String(row['Selling Price'] || 0).replace(/Rs\.?\s*/, '')),
            stock: parseInt(row['Stock'] || 0, 10),
            supplierName: row['Supplier'],
            itemCode: row['Item Code'] || `${Date.now()}-${Math.floor(Math.random() * 10000)}`,
          ...
          const existingProduct = products.find((p) => p.itemName === productData.itemName);

          const url = existingProduct ? `${API_URL}/${existingProduct._id}` : API_URL;
          ...
          return { ok: response.ok, errorMsg };
        });
        ...
        if (failedRequests.length > 0) {
          alert(
            `${failedRequests.length} products could not be uploaded or updated.\nThis may be because some products do not exist in the system for update, or there is an issue with the Excel data. Please check your Excel file and try again.`
          );
        } else {
          alert('Products uploaded/updated successfully!');
        }
        fetchProducts();
      } catch (err) {
        setError(err.message);
    ...
    <div className={`product-repair-list-container ${darkMode ? "dark" : ""}`}>
      ...
      {showReportOptions && (
        <div className="report-modal-overlay" onClick={() => setShowReportOptions(false)}>
          <div
            className={`report-modal-content ${darkMode ? 'dark' : ''}`}
